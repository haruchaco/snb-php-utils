<?php
/**
 * mychaelstyle\storage\File.php class file.
 *
...
class File {
   ...
   */
  public function __construct(\mychaelstyle\Storage $storage,$uri,array $options=array(),$autoCommit=true){
  ...
  public function open($mode='r'){
    if($this->isOpened()){
      throw new \mychaelstyle\Exception('This instance already open another uri file!');
    }
    $this->mode = $mode;
    $this->handle  = @fopen($this->tmp, $this->mode);
    if( false === $this->handle ){
      throw new \mychaelstyle\Exception('Fail to open file '.$this->tmp);
    } else {
      $lr = true;
      ...
      if( false === $lr ){
        throw new \mychaelstyle\Exception('Fail to lock file! '.$this->tmp);
      }
    }
  }
  /**
  ...
  public function putContents($contents){
    if(false === file_put_contents($this->tmp,$contents)){
      throw new \mychaelstyle\Exception('Fail to write local temp file! '.$this->tmp.' '.$this->uri,0,null);
    }
    if($this->auto_commit){
      $this->commit();
    }
   ...
   * @throws mychaelstyle\storage\Exception
   */
  private function checkOpen(){
    if(!$this->isOpened()){
      throw new \mychaelstyle\Exception('This instance has not opened yet!');
    }
  }
}
